...
const materialWeights = Object.keys(materialPalette)
const random = function(colorSpace = "bright") {
            let color = 'white';
            if (colorSpace === 'pastel') {
                color = tinycolor.mix(tinycolor.random().saturate(10), tinycolor('#fff'), amount = 50).toHexString()
            } else if (colorSpace === 'material') {
                var weight = Math.floor(Math.random() * materialWeights.length)
                var index = Math.floor(Math.random() * materialPalette[materialWeights[weight]].length)
                color = materialPalette[materialWeights[weight]][index]
            } else if (colorSpace === 'dull') {
                color = tinycolor.mix(tinycolor.random(), tinycolor('#222'), amount = 50).desaturate(10).toHexString()
            } else if (colorSpace === 'bright') {
                var hue = Math.floor(Math.random() * 255)
                var saturation = 100
                var lightness = 50
                var random_color = tinycolor({
                    h: hue,
                    s: saturation,
                    l: lightness
                })
                color = random_color.toHexString()
            } else if (colorSpace === 'gray') {
                var random_gray = Math.floor(Math.random() * 200) + 20
                color = tinycolor({
                    r: random_gray,
                    g: random_gray,
                    b: random_gray
